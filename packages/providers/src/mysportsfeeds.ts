...
export class MySportsFeedsProjectionProvider implements ProjectionProvider {
  ...
  async getProjections(input: ProjectionRequest): Promise<ProjectionImport> {
    const data = (await getJson(
      `${this.baseUrl}/seasonal_player_projections.json?season=${input.season}`,
      {
        headers: {
    ...
    if (!Array.isArray(data.playerprojections))
      throw new ProviderError(
        "INVALID_RESPONSE",
        "Projection response did not include player projections.",
      );
    return {
      source: "mysportsfeeds",
      retrievedAt: new Date(),
      players: data.playerprojections.flatMap(({ player, stats }) =>
