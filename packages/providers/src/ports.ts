...
export type ProviderErrorCode = "UNAVAILABLE" | "RATE_LIMITED" | "INVALID_RESPONSE" | "AUTHENTICATION_FAILED";
...
export interface AdpRequest { readonly season: number; readonly scoring: "standard" | "half-ppr" | "ppr"; readonly teams: number; }
export interface AdpPlayer { readonly fullName: string; readonly team?: string | undefined; readonly position?: string | undefined; readonly adp: number; }
export interface AdpImport { readonly source: ProviderName; readonly retrievedAt: Date; readonly players: readonly AdpPlayer[]; }
export interface LeaguePlatformProvider { findLeagues(input: { username: string; season: number }): Promise<readonly ExternalLeague[]>; getLeagueSnapshot(input: { leagueId: string }): Promise<LeagueSnapshot>; getDraftSnapshot(input: { draftId: string }): Promise<DraftSnapshot>; }
export interface ProjectionProvider { getProjections(input: ProjectionRequest): Promise<ProjectionImport>; }
export interface AdpProvider { getAdp(input: AdpRequest): Promise<AdpImport>; }
