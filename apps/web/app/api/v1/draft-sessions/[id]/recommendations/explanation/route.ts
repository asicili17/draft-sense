...
function fallback(name: string, confidence: number, factors: Record<string, number>) {
  return {
    ...
    supportingFactors: [
      ...
      `Roster fit: ${(factors.rosterFit ?? 0).toFixed(2)}; coverage: ${(factors.coverage ?? 0).toFixed(2)}; upside: ${(factors.upside ?? 0).toFixed(2)}.`,
      `Scarcity: ${(factors.scarcity ?? 0).toFixed(2)}; roster-completion urgency: ${(factors.completionUrgency ?? 0).toFixed(2)}.`,
    ],
      ...
      .array(
        z.object({
          ...
          factors: z.object({
            ...
            upside: z.number().default(0),
            completionUrgency: z.number().default(0),
            redundancy: z.number().default(0),
            adpValue: z.number(),
